...
import { StatisticItemViewRepository } from '../../repository/services/statistic-item-view-repository.service';
import { StatisticListViewRepository } from '../../repository/services/statistic-list-view-repository.service';
...
import { CarQueryStatisticReqDto } from '../dto/req/car-query-statistic.req.dto';
...
export class CarsService {
  ...
  public async search(
    ...
    const cars = await this.carRepository.search(query);
    await this.statisticViewListRepository.save(
      cars[0].map((item) =>
        this.statisticViewListRepository.create({ car_id: item.car_id }),
      ),
    );
    return cars;
  }
  ...
  public async getCar(
    car_id: string,
    query: CarQueryStatisticReqDto,
  ): Promise<any> {
    const car = await this.carRepository.getCar(car_id, query);
    await this.statisticViewItemRepository.save(
      this.statisticViewItemRepository.create({ car_id }),
    );
    return car;
  }
