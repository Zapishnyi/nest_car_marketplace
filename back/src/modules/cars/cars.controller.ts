...
import { UserPlanGuard } from '../../common/guards/user-plan.guard';
...
export class CarsController {
  ...
  @UseGuards(JwtAccessGuard, OwnershipGuard, UserPlanGuard)
  @ApiBearerAuth()
  @ApiNotFoundResponse({
    example: {
      statusCode: 404,
  ...
  @ApiForbiddenResponse({ description: 'Forbidden' })
  @Get(':car_id/statistic/')
  public async getCarStatistic(
    @Param('car_id', ParseUUIDPipe) car_id: string,
    @Query() query: CarQueryStatisticReqDto,
  ): Promise<CarResDto> {
    return this.carPresenter.RawToResponseDto(
      await this.carsService.getCar(car_id, query),
    );
